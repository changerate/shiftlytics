...
export default function ProfilePanel({ isOpen, onClose, userId }) {
    ...
    return (
        <div className="fixed inset-0 z-50">
            ...
            {/* Nested create wage modal */}
            <ProfileCompletionPrompt
                isOpen={showCreateWage}
                onClose={() => setShowCreateWage(false)}
                userId={userId}
                onCreated={() => { setShowCreateWage(false); handleCreatedWage(); }}
            />
        </div>
    );
}
